...
条件分岐
 ├─ 1: 情報を残すべき → リポジトリへ反映
 │      (新規: そのディレクトリを repo 化 / 既存: 該当リポへ。新規か既存かはここで判断)
...
| git の単位 | 基本は 1 ディレクトリ = 1 リポ。「残すべき」と判断した時点で repo 化する。ただし既存リポに属する情報はそのリポへ反映し、新規 / 既存の判断は分岐 1 の入口で行う |
...
| repo 化 | 条件分岐 1 の入口。新規 / 既存リポの判断を支援し、新規なら git init + gh repo create + push、既存なら該当リポへの反映に誘導 |
