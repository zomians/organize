...
- **「subagent を使わない」とは矛盾しない**。自動実行モードの subagent 不使用は、organize が自フェーズ（create-issue → commit → create-pr）を subagent / worktree に委譲しない＝中央オーケストレータ化しない、という制約である。`/code-review` はフォアグラウンドで呼ぶ成果物ツールで、内部の finder/verifier fan-out は実装詳細にすぎず、成果物（findings）が返り人間が事後検証できる以上「丸投げ」に当たらない。起動時の許可プロンプトは自動実行が温存する per-user backstop（ADR-0002）であり、畳む 3 ゲートには含めない。
