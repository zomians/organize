...
入口は 2 つ — 頭に悩みが**ある**ときは壁打ち（spar）、悩みがまだ**無く**何を整理するか自体を探すときは巡回（patrol）から入る。頭が整理できたら、その成果を「どう残すか／残さないか」が出口として分岐する。

```
やることを探す（GitHub 巡回で「考えた方が良いこと」を発掘）  ← 悩みが無いときの入口
   ↓  patrol（→ spar / 直接アクションへ）
現状把握（壁打ち / 必要ならリサーチ）                       ← 悩みがあるときの入口
   ↓  spar
[ 出口の分岐 ]
...
| skill | 役割 |
|---|---|
| **spar** | 整理の入口。計画を鍛える壁打ちを一問一答・推奨案付きで行い、矛盾・曖昧さ・未検証の前提を突く。surface した signal から必要な doc を逆引きし、本文の下書きを提示して赤入れさせながら残し、最後に出口へ案内する。 |
