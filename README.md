...
入口は 2 つ — 整理したい話題が**手にある**ときは壁打ち（spar）、話題がまだ**無く**何を整理するか自体を探すときは点検（patrol）から入る。両者は同じ「疑う」エンジンを共有し、話題が手にあるか／これから掘るかだけが差す。頭が整理できたら、その成果を「どう残すか／残さないか」が出口として分岐する。

```
やることを探す（既存の repo/dir を深く読み、疑い、整理すべき話題を掘る）  ← 話題が無いときの入口
   ↓  patrol: 把握 → 疑う → 探す（→ 出口 / spar で深掘り）
現状把握（壁打ち / 必要ならリサーチ）                                     ← 話題が手にあるときの入口
   ↓  spar
[ 出口の分岐 ]
...
| **patrol** | 整理の第二の入口。既存の repo/ディレクトリを1つ深く読み（把握）、敵対的に疑い（叩く）、「整理すべき話題」を掘り起こす（話題化）。organize の核心（言語化 / GitHub flow）を疑うのレンズにし、放置（issue/PR/branch/in-flight）も flow の綻びとして拾う。揮発リストを置いて人間が 1 件選び、出口か spar へ渡す。話題がまだ手に無いときの入口。 |
...
何を整理するか自体が定まっていない（話題がまだ手に無い）ときは、点検（patrol）から入る:
...
これらで **patrol** が発火し、既存の repo／ディレクトリを深く読んで敵対的に疑い、「整理すべき話題」を掘り起こす。1 件選べば出口（残す）か spar での深掘りへ繋がる。
