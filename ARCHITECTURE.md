...
情報を残すか否かの判断は出口の分岐（A〜E）で完結しており、ファイル命名等で再表現しない。唯一の例外が出口 E の `handoff.md` で、セッション間の引き継ぎ専用にリポ直下へ置くが commit はしない。issue / PR 下書きなどの作業ファイルはそもそもリポに置かない（/tmp に書き、用が済んだら消す）。
