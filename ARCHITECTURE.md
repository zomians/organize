...
subagent への委譲は、**レビュー可能な成果物が返る実行タスク**に限る。spar（入口＝入力の擦り合わせ）と成果物レビュー（出口＝出力の検証）が、見えない中間のやり取りの代わりを果たす。人間が両端に集約して居続けることで「伴走」を壊さない。中間（subagent の自律実行）は不透明でよく、人間は入口と出口にだけ居る。

**委譲 ≠ オーケストレーション ≠ 丸投げ**。各 skill は疎結合のまま、中央のオーケストレータは導入しない（委譲 ≠ オーケストレーション）。成果物が返らない「真の探索」は subagent に出さず spar で人間がやる（委譲 ≠ 丸投げ）。これが境界線。
...
情報を残すか否かの判断は出口の分岐（残す / 何も残さず / handoff）で完結しており、ファイル命名等で再表現しない。唯一の例外が handoff 出口の `handoff.md` で、実行者間（次セッションの自分 / subagent）の引き継ぎ専用にリポ直下へ置くが commit はしない。中断（次セッションの自分へ渡す）と委譲（subagent へ渡す）は本質的に同じ「実行者間の引き継ぎ」で、同じ `handoff.md` で繋がる。揮発・commit しないの原則は維持する。issue / PR 下書きなどの作業ファイルは作らない（本文は会話で提示し、コマンドへは stdin で渡す）。
...
出口の先で実行を委譲するときは、委譲タイプごとに入口（擦り合わせ）と出口（検証）が決まる:

| タイプ | 入力（擦り合わせ） | 出力（検証） | 戻し点 |
|---|---|---|---|
| 残す実行 | issue（レビュー済み仕様） | PR | PR 目視 → 人間が merge |
| 軽い委譲 | handoff.md | 読める成果物（diff / 要約 / draft） | 成果物レビュー |
| 真の探索（成果物なし） | — | — | subagent に出さず spar で人間がやる |
