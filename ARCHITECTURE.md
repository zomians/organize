...
入口は 2 つある。頭に悩み・課題が**ある**ときは spar（内から引き出す）。悩みがまだ**無く**、何を整理 / 作業するか自体を探すときは patrol（外を巡回して持ち込む）。patrol の出力は spar かアクションへ放射状につながる（探す → 鍛える → 残す）。
...
- **粒度は signal に従属させる**。doc 単位でも固定行数でもなく、surface した signal が指す範囲だけを draft にする。逆引きが既に「走査ではなく signal が範囲を決める」設計なので、新しい判定軸が要らない。
- **§1（鍛え方）には置かない**。§1 に並ぶのは「既に出た発言を叩く」手（矛盾指摘・正準化・シナリオ・コード照合）で、draft first は発言が無い所を埋める手。混ぜると spar の輪郭がぼやける（ADR-0001 の系譜）。
- **patrol には波及させない**。patrol は結論を出さず出力をファイル化しない skill で、doc draft を持ち込むとその identity とぶつかる。§2 に置けば spar 限定で閉じる。

### patrol（第二の入口・探す）

整理対象を外から探す入口 skill。spar が頭の中の悩みを内から引き出すのに対し、patrol は GitHub リポを巡回して、まだ言語化されていない論点を外から持ち込む（情報の向きが逆）。素プロンプトでも plugin のプラットフォーム化でもなく、Claude Code の skill 追加で実現する（「層を増やすな」に沿う。巡回先が増えても skill を足せばよい）。

**判定は organize の核心から演繹する診断レンズ**。リポの「複雑さ × 活動量」に対する**言語化負債 + flow 負債**のギャップを測り、大きい順に「考えた方が良いこと」として置く。判定基準を場当たりなヒューリスティクスにせず核心から導くことで、ドリフトしない。

**lazy 原則を守る弁別子**: 「doc が無い」では flag しない。doc 不在そのものは負債でない（lazy 作成が正常）。負債は活動量でスケールし、活動の無い若いリポは自然に足切りされる。patrol は督促チェッカーではなく、「signal が repo の歴史で繰り返し surface しているのに doc / flow に乗っていない」状態の検出器。

**コスト対策の二段構え**: Pass1 で横断的に安価にランク付けし、Pass2 で上位だけ精読する（全リポ精読はコストで破綻するため）。

**patrol は結論を出さない**。揮発するランク付きリストを出し、人間が 1 件選ぶ（何を整理するかの選択権は人間が握る）。下流は 2 系統に振り分かる — 言語化負債／未解決論点は spar（鍛える）へ、flow 負債で打ち手が明確なもの（放置 PR 等）は spar を経ず直接アクション（create-pr 続行 / create-issue / close）へ。出力をファイル化しないのは「作業ファイルを作らない」規律に沿う。

巡回対象は引数で渡す。config ファイルは作らない・要求しない（空でも即使える思想／設定ドリフトを避ける）。
