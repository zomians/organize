...
入口は壁打ち skill（spar）1 つ。頭に悩み・課題があるときに spar から入る。
...
- **§1（鍛え方）には置かない**。§1 に並ぶのは「既に出た発言を叩く」手（矛盾指摘・正準化・シナリオ・コード照合）で、draft first は発言が無い所を埋める手。混ぜると spar の輪郭がぼやける（→ ADR-0004「§1 に置かない理由」）。
