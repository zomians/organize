...
情報を残すか否かの判断は出口の分岐（残す / 何も残さず / handoff）で完結しており、ファイル命名等で再表現しない。唯一の例外が handoff 出口の `handoff.md` で、セッション間の引き継ぎ専用にリポ直下へ置くが commit はしない。issue / PR 下書きなどの作業ファイルは作らない（本文は会話で提示し、コマンドへは stdin で渡す）。
